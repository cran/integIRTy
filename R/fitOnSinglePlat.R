...
function(data, model=2, guessing=FALSE, 
  ...
  if(model==1) {
    tempFit <- try(rasch(temp, constraint = cbind(ncol(temp) + 1, 1), ...),
                   silent=TRUE)
  }
  if(model==2) {
  ...
  return(list(fit=tempFit, model=model, guessing=guessing,
              sampleIndices=sampleIndices, geneIndices=geneIndices))
}
